...
import { Container, Typography, Box, Paper, TextField, Button, CircularProgress, Grid, Card, CardMedia, CardContent, Table, TableBody, TableCell, TableHead, TableRow, TableContainer, FormControl, InputLabel, Select, MenuItem } from '@mui/material';
...
const TeacherDashboardScreen = () => {
  ...
  const handleCreateExam = async () => {
    try {
      ...
      refetchExams(); // immediately refresh the dashboard component
    } catch (err) {
              ...
              <Paper sx={{ p: 4, borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', bgcolor: '#ffffff' }}>
                ...
                <TextField fullWidth label="Topic (e.g. JavaScript Arrays)" margin="normal" value={topic} onChange={(e) => setTopic(e.target.value)}
                  sx={{ 
                    ...
                    '& .MuiOutlinedInput-root': { borderRadius: '10px', '& fieldset': { borderColor: "#ddd" }, '&:hover fieldset': { borderColor: '#6c63ff' }, '&.Mui-focused fieldset': { borderColor: '#6c63ff', boxShadow: '0 0 0 4px rgba(108, 99, 255, 0.1)' } } 
                  }} />
                ...
                <FormControl fullWidth margin="normal" sx={{
                  ...
                  '& .MuiOutlinedInput-root': {
                    ...
                    '&.Mui-focused fieldset': { borderColor: '#6c63ff', boxShadow: '0 0 0 4px rgba(108, 99, 255, 0.1)' }
                  },
                ...
                <Button variant="contained" fullWidth sx={{ 
                  mt: 3, py: 1.5, borderRadius: '10px', textTransform: 'none', fontSize: '1rem', fontWeight: 600, color: '#ffffff',
                  background: 'linear-gradient(135deg, #6c63ff, #7f53ac)',
                  '&:hover': { background: 'linear-gradient(135deg, #5b54e5, #6a459b)', transform: 'translateY(-2px)', boxShadow: '0 6px 20px rgba(108, 99, 255, 0.4)' },
            ...
            <Grid item xs={12} md={6}>
              <Paper sx={{ p: 4, borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', bgcolor: '#ffffff' }}>
                ...
                <Button variant="contained" fullWidth sx={{ 
                  ...
                  '&:hover': { background: 'linear-gradient(135deg, #5b54e5, #6a459b)', transform: 'translateY(-2px)', boxShadow: '0 6px 20px rgba(108, 99, 255, 0.4)' },
                  transition: 'all 0.3s ease'
                }} onClick={handleCreateExam} disabled={isCreating || generatedQuestions.length === 0}>
                ...
                <TableContainer component={Paper} sx={{ borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', mb: 5, overflow: 'hidden' }}>
                  <Table>
                    <TableHead sx={{ bgcolor: '#ffffff' }}>
                      <TableRow>
                        <TableCell sx={{ fontWeight: 600, color: '#666' }}>Student Name</TableCell>
                        <TableCell sx={{ fontWeight: 600, color: '#666' }}>Exam Title</TableCell>
                        <TableCell sx={{ fontWeight: 600, color: '#666' }}>Score</TableCell>
                        <TableCell sx={{ fontWeight: 600, color: '#666' }}>AI Cheat Summary</TableCell>
                        <TableCell sx={{ fontWeight: 600, color: '#666' }}>Time Analysis</TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {results?.map((res, index) => {
                        ...
                        return (
                          <TableRow key={res._id} sx={{ bgcolor: index % 2 === 0 ? '#ffffff' : '#f9fafb', transition: '0.2s', '&:hover': { bgcolor: '#f1f3f6' } }}>
                            <TableCell sx={{ color: '#333', fontWeight: 500 }}>{res.student?.name || 'Unknown'}</TableCell>
                            <TableCell sx={{ color: '#333' }}>{res.exam?.title || 'Unknown'}</TableCell>
                            <TableCell sx={{ color: '#333', fontWeight: 600 }}>{res.score} / {res.totalQuestions}</TableCell>
                            <TableCell sx={{ 
                              color: res.reportSummary?.includes('0') ? '#2e7d32' : '#d32f2f',
                              fontWeight: 500
                            }}>
                              {res.reportSummary}
                            </TableCell>
                            <TableCell sx={{ color: '#333' }}>{Math.floor(totalTime / 60)}m {totalTime % 60}s</TableCell>
                          </TableRow>
                        )
                      })}
                    </TableBody>
